...
func (ap *audioPanel) handle(event tcell.Event) (changed, quit bool) {
	...
	case *tcell.EventKey:
		...
		case 'z':
			speaker.Lock()
			ap.resample.SetRatio(ap.resample.Ratio() * 15 / 16)
			speaker.Unlock()
			return true, false
		case 'x':
			speaker.Lock()
			ap.resample.SetRatio(ap.resample.Ratio() * 16 / 15)
			speaker.Unlock()
			return true, false
		}
	}
...
/*
CMD playlist by go
to start you need go run main.go + songName.mp3
*/
